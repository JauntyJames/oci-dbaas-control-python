#
# oci-dbaas-control-python version 1.0.
#
...
def instance_status(db_client, instance_id):
    return db_client.get_db_node(instance_id).data.lifecycle_state


def instance_start(db_client, instance_id):
    print('Starting Instance: {}'.format(instance_id))
    try:
        if instance_status(db_client, instance_id) in 'STOPPED':
            try:
                resp = db_client.db_node_action(instance_id, 'START')
    ...
    return instance_status(db_client, instance_id)

def instance_stop(db_client, instance_id):
    print('Stopping Instance: {}'.format(instance_id))
    try:
        if instance_status(db_client, instance_id) in 'AVAILABLE':
            try:
                resp = db_client.db_node_action(instance_id, 'STOP')
...
def handler(ctx, data: io.BytesIO=None):
    ...
    if command == 'status':
        resp = instance_status(db_client, instance_ocid)
    elif command == 'start':
        resp = instance_start(db_client, instance_ocid)
    elif command == 'stop':
        resp = instance_stop(db_client, instance_ocid)
    else:
